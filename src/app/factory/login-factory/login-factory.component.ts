...
@Component({
  selector: 'app-login-factory',
  templateUrl: './login-factory.component.html',
  styleUrls: ['./login-factory.component.css']
})
export class LoginFactoryComponent implements OnInit {
  ...
  onSubmit(){
    this.submitted=true;
      this.authService.loginfactory(this.loginForm.value).subscribe(result=>{
        console.log(result);
      ...
      this.router.navigate(['factory_page']);
    ...
    onReset():void{
      ...
      responseHandler(data:any){
        console.log(data.tokenfact);
          ...
          this.token.handleDatafact(data.tokenfact,data.idfact);
